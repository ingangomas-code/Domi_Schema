...
Al iniciar sesión, el expediente se sincroniza automáticamente con `public.ai_workspaces`, con clave `(owner_id, project_id)`. Se recupera al abrir el proyecto y se guarda tras cargar fuentes, generar o revisar recomendaciones. IndexedDB mantiene una copia por cuenta y proyecto; los cambios pendientes se reintentan al abrir el proyecto o recuperar conexión. Si una copia local pendiente coincide con cambios más recientes en otro dispositivo, se conserva y se muestra el conflicto. **Guardar nube** y **Abrir nube** permanecen como controles manuales. RLS restringe los datos al propietario. Se verificaron inserción, actualización y lectura con el rol `authenticated` en una transacción revertida.

Las cargas reutilizan la sesión de la aplicación y renuevan el token una vez ante un 401. Los fallos de sesión, permisos o conexión detienen el lote y permiten reintentar los archivos pendientes mientras la pestaña siga abierta. Solo los fallos de validación/extracción de un archivo se incluyen en “omitidos”. Los estados del mapa y del expediente muestran el resultado real del guardado, por separado.
