(function (root) {
  ...
  function validate(input) {
    ...
    if (!Array.isArray(input.modules) || !Array.isArray(input.entities) || !Array.isArray(input.relationships)) fail();
    if (input.modules.length > 100 || input.entities.length > 1000 || input.relationships.length > 5000) fail();
    ...
    const entities = input.entities.map(n => {
      ...
      return {id:id(n.id),module:n.module,name,label:str(n.label,160),description:str(n.description,4000),x:n.x,y:n.y,
    ...
    return{version:1,title:str(input.title,100,'Mi mapa')||'Mi mapa',modules,entities,relationships};
  }
  ...
  root.SchemaModel={uid,clone,example,validate,connect,removeNode,duplicateNode,sides};
