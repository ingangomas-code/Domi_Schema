(function(root){
  ...
  function validateDraft(raw,context,grounded,options={}){
    ...
    const tolerant=options.tolerant===true,citationIssues=[];
    const string=(x,max=160)=>typeof x==='string'&&x.trim()&&x.length<=max;
    const byId=new Map(context.map(c=>[c.id,c]));
    const cite=(e,where)=>{if(!Array.isArray(e)||e.length>12){if(!tolerant)throw new Error('Evidencias inválidas.');citationIssues.push({where,reason:'Formato de evidencia inválido.'});return [];}if(grounded&&!e.length){if(!tolerant)throw new Error('Se rechazó un elemento sin evidencia.');citationIssues.push({where,reason:'El elemento no incluyó evidencia.'});return [];}const valid=[];for(const ref of e){const c=byId.get(ref?.chunkId),quote=ref?.quote;if(!c||!string(quote,1800)||quote.trim().length<8||!c.text.includes(quote)){if(!tolerant)throw new Error('Una cita no existe en los fragmentos recuperados.');citationIssues.push({where,chunkId:ref?.chunkId||'',quote:typeof quote==='string'?quote.slice(0,180):'',reason:'La cita no coincide literalmente con los fragmentos recuperados.'});continue;}valid.push({chunkId:c.id,quote,name:c.name,locator:c.locator});}if(grounded&&!valid.length&&e.length){if(!tolerant)throw new Error('Se rechazó un elemento sin evidencia válida.');}return valid;};
    const nodes=raw.nodes.map(n=>{if(!n||!string(n.id,100)||!string(n.name,100)||!Array.isArray(n.fields)||n.fields.length>40)throw new Error('Nodo inválido.');const evidence=cite(n.evidence||[],'Nodo '+n.name);return {id:n.id,name:n.name,group:typeof n.group==='string'?n.group.slice(0,100):'',evidence,unverified:grounded&&!evidence.length,fields:n.fields.map(f=>{if(!string(f.name,100)||!string(f.type,50))throw new Error('Campo inválido.');const evidence=cite(f.evidence||[],'Campo '+f.name+' de '+n.name);return {name:f.name,type:f.type,evidence,unverified:grounded&&!evidence.length};})};});
    const ids=new Set(nodes.map(n=>n.id));if(ids.size!==nodes.length)throw new Error('IDs repetidos.');
    const edges=raw.edges.map((e,i)=>{if(!ids.has(e.from)||!ids.has(e.to)||!string(e.label,160))throw new Error('Relación inválida.');const evidence=cite(e.evidence||[],'Relación '+e.label);return {id:'edge_'+i,from:e.from,to:e.to,label:e.label,evidence,unverified:grounded&&!evidence.length};});
    const recommendations=(Array.isArray(raw.recommendations)?raw.recommendations:[]).slice(0,20).map((r,i)=>{if(!string(r.text,1200))throw new Error('Recomendación inválida.');const evidence=cite(r.evidence||[],'Recomendación '+r.text);return {id:'rec_'+i,text:r.text,evidence,unverified:grounded&&!evidence.length,reviewed:false,accepted:false};});
    const recommendedLanguages=(Array.isArray(raw.recommendedLanguages)?raw.recommendedLanguages:[]).slice(0,12).map((r,i)=>{if(!string(r.name,80)||!string(r.reason,1200))throw new Error('Lenguaje recomendado inválido.');const evidence=cite(r.evidence||[],'Lenguaje '+r.name);return {id:'language_'+i,name:r.name,reason:r.reason,evidence,unverified:grounded&&!evidence.length};});
    return {title:raw.title.slice(0,100),nodes,edges,recommendations,recommendedLanguages,grounded,limitations:typeof raw.limitations==='string'?raw.limitations.slice(0,2500):'',citationIssues,createdAt:new Date().toISOString()};
  }
